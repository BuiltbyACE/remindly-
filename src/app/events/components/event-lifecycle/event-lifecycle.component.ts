...
@Component({
  ...
  template: `
    <div class="py-6">
      ...
      <!-- Mobile Vertical Checklist Timeline -->
      <div class="block sm:hidden relative pl-8 space-y-4">
        <!-- Vertical Line Track -->
        <div class="absolute left-4 top-2 bottom-2 w-0.5 bg-gray-150 rounded-full"></div>
        <!-- Vertical Progress Line -->
        <div
          class="absolute left-4 top-2 w-0.5 bg-gradient-to-b from-blue-500 via-indigo-500 to-purple-500 rounded-full transition-all duration-500 ease-out"
          [style.height.%]="progressWidth()"
        ></div>

        @for (step of steps(); track step.status) {
          <div class="flex items-center gap-3.5 relative py-0.5">
            <!-- Step Circle -->
            <div
              class="w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-300 shadow-sm cursor-default z-10"
              [class]="stepClass(step)"
            >
            ...
            <div class="flex flex-col">
              ...
              @if (step.position === currentPosition()) {
                <span class="text-[9px] text-indigo-500 font-medium animate-pulse">Current Status</span>
              } @else if (step.position < currentPosition()) {
                <span class="text-[9px] text-emerald-500 font-medium">Completed</span>
              }
            </div>
          </div>
        }
...
export class EventLifecycleComponent {
  ...
  protected readonly currentPosition = () => {
