      ...
      @if (!store.loading() && !store.error()) {
        <div class="bg-white rounded-lg border border-[var(--color-border)] shadow-sm overflow-hidden">
          @if (store.hasEvents()) {
            ...
            <div class="block lg:hidden divide-y divide-[var(--color-border)]">
              @for (event of store.events(); track event.id) {
                <div #eventRow
                     class="p-4 hover:bg-[var(--color-surface-alt)] transition-colors flex items-start gap-3"
                     [class.bg-blue-50]="selectedIds().includes(event.id)">
                  <!-- Checkbox -->
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-1">
                      <a [routerLink]="['/events', event.id]" class="text-blue-600 hover:text-blue-800">
                        {{ event.title }}
                      </a>
                    </h3>
                  ...
                  <div class="flex-shrink-0 flex items-center gap-1.5 self-center">
                    <a
                      [routerLink]="['/events', event.id]"
                      class="p-2 text-blue-600 hover:text-blue-800 bg-blue-50 rounded-lg transition-colors"
                      aria-label="View {{ event.title }}"
                    >
                    ...
                    <a
                      [routerLink]="['/events', event.id, 'edit']"
                      class="p-2 text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] bg-gray-50 rounded-lg transition-colors"
                      aria-label="Edit {{ event.title }}"
                    >
                      <svg aria-hidden="true" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
